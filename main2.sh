...
wget https://raw.githubusercontent.com/Jatimpark/multi/main/uye/setup.sh >/dev/null 2>&1
chmod +x setup.sh >/dev/null 2>&1
screen -S ssh-vpn ./setup.sh >/dev/null 2>&1
...
function add_domain() {
    ...
    echo -e "${red}    ${NC} ${green} CUSTOM SETUP DOMAIN VPS     ${NC}"
...
"
    ...
    echo "    │   - Script Nbc Group Arya Blitar 087721815317       │"
    echo "    └─────────────────────────────────────────────────────┘"
